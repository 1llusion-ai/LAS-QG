...
class Neo4jClient:
    ...
    def create_vector_index(self, index_name: str = "chunk_vector", dimension: int = 1024) -> bool:
        ...
        with driver.session() as session:
            try:
                ...
                """
                session.run(cypher)
                print(f"[Neo4j] Vector Index '{index_name}' 创建成功")
                return True
            except Exception as e:
                print(f"[Neo4j] 创建 Vector Index 失败: {e}")
                return False

    def create_fulltext_index(self, index_name: str = "chunk_fulltext") -> bool:
        driver = self._get_driver()
        with driver.session() as session:
            try:
                cypher = f"""
                ...
                """
                session.run(cypher)
                print(f"[Neo4j] Fulltext Index '{index_name}' 创建成功")
                return True
            except Exception as e:
                print(f"[Neo4j] 创建 Fulltext Index 失败: {e}")
                return False
    ...
    def bm25_search(self, query: str, top_k: int = 5) -> list:
        ...
        with driver.session() as session:
            ...
            RETURN c.chunk_key AS chunk_key, c.chunk_id AS chunk_id,
                   ...
                   c.source_text AS source_text, score
            ORDER BY score DESC
            LIMIT $top_k
            """
            try:
                cursor = session.run(cypher, search_query=query, top_k=top_k)
                ...
                print(f"[Neo4j] BM25 搜索失败: {e}")
                results = self._fallback_keyword_search(session, query, top_k)
        return results

    def _fallback_keyword_search(self, session, query: str, top_k: int) -> list:
        results = []
        try:
            cypher = """
            MATCH (c:Chunk)
            WHERE c.source_text CONTAINS $keyword
            RETURN c.chunk_key AS chunk_key, c.chunk_id AS chunk_id,
                   c.doc_title AS doc_title, c.article AS article,
                   c.source_text AS source_text
            LIMIT $top_k
            """
            cursor = session.run(cypher, keyword=query, top_k=top_k)
            for record in cursor:
                results.append({
                    "chunk_key": record.get("chunk_key", ""),
                    "chunk_id": record.get("chunk_id", 0),
    ...
    def similarity_search(
        ...
        with driver.session() as session:
            # 使用 Neo4j 原生 Vector Index 查询
            cypher = """
            CALL db.index.vector.queryNodes('chunk_vector', $top_k, $query_embedding)
            YIELD node AS c, score
            RETURN c.chunk_key AS chunk_key, c.chunk_id AS chunk_id,
                   c.doc_title AS doc_title, c.article AS article,
                   c.source_text AS source_text, score AS similarity
            """
            try:
                ...
                print(f"[Neo4j] Vector Index 查询失败: {e}")
                print("[Neo4j] 尝试使用备用搜索方式...")
                # 备用：使用关键词搜索
                results = self._fallback_search(session, query_embedding, top_k)

        return results

    def _fallback_search(self, session, query_embedding: list[float], top_k: int) -> list:
        """备用搜索：直接返回一些 Chunk"""
        results = []
        try:
            cypher = """
            ...
            LIMIT $top_k
            """
            cursor = session.run(cypher, top_k=top_k)
            for record in cursor:
                results.append({
                    "chunk_key": record.get("chunk_key", ""),
                    "chunk_id": record.get("chunk_id", 0),
